import { Name, Avatar, Address, AvatarByName } from "@paperclip-labs/whisk-sdk/identity";
...
The address for a name and resolver, think of this as the reverse of the `Name` component.

:::info
In cases where there can be multiple addresses for a given name (eg. Farcaster), the first address will be used.
:::

:::code-group 
<div data-title="Preview">
    <Address name="vitalik.eth" resolver="ens" />
</div>

```tsx [Code] twoslash 
import { Address } from "@paperclip-labs/whisk-sdk/identity";
...
| `resolver` | [`IdentityResolver`](#identityresolver) | Yes | Resolver the name is using (`ens` for .eth, `base` for .base.eth, `farcaster` for farcaster names, etc.). |
| `className` | `string` | No | Custom CSS class names. |

Example customizing with tailwindCSS:
:::code-group 
<div data-title="Preview">
    <Address name="vitalik.eth" resolver="ens" className="text-sm text-red-200" />
</div>
...
The Avatar for a name and resolver.

:::code-group 
<div data-title="Preview">
    <AvatarByName name="stani" size={32} resolver="lens" />
</div>

```tsx [Code] twoslash 
import { AvatarByName } from "@paperclip-labs/whisk-sdk/identity";
...
:::code-group 
<div data-title="Preview">
    <AvatarByName name="stani" size={32} resolver="lens" className="hover:scale-125 transition-transform rounded-md" />
</div>
...
```tsx twoslash
import { useAddress } from "@paperclip-labs/whisk-sdk/identity";

const { data: address, isLoading } = useAddress({ name: "stani", resolver: "lens" });
```
...
```tsx twoslash
import { useAvatarByName } from "@paperclip-labs/whisk-sdk/identity";

const { data: avatar, isLoading } = useAvatarByName({ name: "stani", resolver: "lens" });
```
...
```tsx twoslash
import { getAddress } from "@paperclip-labs/whisk-sdk/identity/core";

const address = await getAddress("", { name: "stani", resolver: "lens" })
```

| Params | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `apiKey` | `string` | Yes | Whisk API key, can be empty string for now. |
| `params` | `{name: string, resolver: IdentityResolver}` | Yes | Same params as [`useAddress`](#useAddress). |


### getAvatarByName

```tsx twoslash
import { getAvatarByName } from "@paperclip-labs/whisk-sdk/identity/core";

const avatar = await getAvatarByName("", { name: "stani", resolver: "lens" })
```

| Params | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `apiKey` | `string` | Yes | Whisk API key, can be empty string for now. |
| `params` | `{name: string, resolver: IdentityResolver}` | Yes | Same params as [`useAvatarByName`](#useAvatarByName). |
